...
/* SVG Icons for sidebar - clean Heroicons outline style */
function SidebarIcon({ name, active }: { name: string; active: boolean }) {
  const strokeW = 1.5;
  const cls = `w-5 h-5 flex-shrink-0 ${active ? "text-dz-green" : "text-dz-text-muted"}`;

  switch (name) {
    case "home":
      return (
        <svg className={cls} viewBox="0 0 24 24" fill={active ? "currentColor" : "none"} stroke={active ? "none" : "currentColor"} strokeWidth={strokeW}>
          {active
    ...
    case "send":
      return (
        <svg className={cls} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={strokeW}>
  ...
  return (
    <div className="min-h-screen bg-subtle-gradient">
      ...
      <div className="flex max-w-7xl mx-auto">
        ...
        <aside
          className={`
            fixed md:sticky md:top-14 left-0 top-14 z-30
            w-64 h-[calc(100vh-3.5rem)] bg-white/95 backdrop-blur-sm border-r border-dz-border/20
        ...
        >
              ...
              return (
                <Link
                  ...
                  className={`
                    ...
                    ${
                      active
                        ? "bg-dz-green/8 text-dz-green"
                        : "text-dz-text-secondary hover:bg-dz-cream/60 hover:text-dz-text"
                    }
                  `}
                >
                  {/* Active left indicator bar */}
                  {active && (
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 bg-dz-green rounded-r-full" />
                  )}
          ...
          <div className="mt-auto p-4">
            <div className="bg-gradient-to-br from-dz-green-darkest to-dz-dark rounded-2xl p-4 text-white">
        ...
        {/* Mobile sidebar (shows on toggle) */}
        <aside
          className={`
            fixed left-0 top-14 z-30
            w-64 h-[calc(100vh-3.5rem)] bg-white border-r border-dz-border/20
            transition-transform duration-300 ease-in-out
            overflow-y-auto
            ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}
        ...
        {/* Main content */}
        <main className="flex-1 min-h-[calc(100vh-3.5rem)] pb-24 md:pb-6 px-4 py-4 md:px-6 md:py-6 animate-fade-in">
          {children}
        </main>
      </div>
